...
namespace Faker
{
    public static class Internet
    {
        ...
        {
            return Regex.Replace(name, @"[^\w]+", new MatchEvaluator(x => new[] { ".", "_" }.Random())).ToLower();
        }
        ...
        {
            return Company.Name().Split(' ').First().AlphanumericOnly().ToLower();
        }
        ...
        {
            () => Name.First().AlphanumericOnly().ToLower(),
            () => string.Format("{0}{1}{2}", Name.First().AlphanumericOnly(), 
                new [] { ".", "_" }.Random(), Name.Last().AlphanumericOnly()).ToLower()
        };
    }
}